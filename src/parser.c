...
//Remover comillas de una cadena si hay
void remover_comillas(char *args){
    if(args == NULL){return;}
    size_t len = strlen(args);

    // Si el argumento empieza y termina con comillas se elimian
...
int parse_line(char *line, char **args, int *in_background) {
    ...
    *in_background = 0; //Se inicializa en 0 para cada nueva lectura
    ...
    char *token = strtok_r(line, " \t\n\r", &operador); //Pimer corte de la cadena
    ...
    //Asegurar que args termine en NULL, permite usar parse_line repetidimente en una cadena
    args[i] = NULL;

    return operador_detectado;
}